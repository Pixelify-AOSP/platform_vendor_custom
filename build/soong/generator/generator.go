...
func init() {
	android.RegisterModuleType("ascp_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = ascpExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = ascpExpandVariables(ctx, dep_file)
	...
	cmd := ascpExpandVariables(ctx, String(g.properties.Cmd))
